(() => {
  ...
  async function loadLeaderboard() {
    ...
    try {
      const r = await fetch('/api/scores?game=breakout&limit=25', {
        cache: 'no-store',
      });
  ...
  const sBeep = (i) => {
    if (i === 3) tone(360, 0.1, 'square', 0.06);
    else if (i === 2) tone(520, 0.1, 'square', 0.06);
  ...
  function getHS() {
    try {
      const a = JSON.parse(
        localStorage.getItem('breakout_highscores_v2') || '[]',
      );
  ...
  function saveHS(arr) {
    localStorage.setItem('breakout_highscores_v2', JSON.stringify(arr));
  }
  function isTop10(sc) {
    const a = getHS();
    return a.length < 10 || sc > a[a.length - 1].s;
  }
  ...
  function render() {
    ...
    if (paused) {
      ...
      ctx.textAlign = 'center';
      ctx.fillText(
        'Paused — Space resume • R restart • Enter start',
        W / 2,
        H / 2,
      );
    }
    ...
    if (countdown > 0) {
      ...
      ctx.globalAlpha = 1;
      ctx.fillStyle = color;
      ctx.font = 34 * (1 + 0.3 * (1 - t)) + 'px Inter';
      ctx.textAlign = 'center';
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if ([' ', 'Enter', 'r', 'R', 'm', 'M', 'f', 'F'].includes(e.key))
      ensureAudio();
  ...
  function updateFsBtn() {
    if (fsBtn) fsBtn.textContent = isFs() ? '🗗 Windowed' : '⛶ Fullscreen';
  }
